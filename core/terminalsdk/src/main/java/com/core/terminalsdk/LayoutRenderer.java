...
public class LayoutRenderer {
    ...
    /**
     * Renders the swap with custom text into a bitmap
     * @return A bitmap of size 428x142 pixels containing the rendered layout
     */
    public Bitmap renderSwap() {
        // Inflate the layout
        LayoutInflater inflater = LayoutInflater.from(context);
        View view = inflater.inflate(R.layout.swap_terminal_layout, null);

        int accentColor = getColorForRender();

        //Tint icons
        ImageView copyIcon = view.findViewById(R.id.swap_icon);
        if (copyIcon != null) {
            copyIcon.setColorFilter(accentColor, PorterDuff.Mode.SRC_IN);
        }

        // Re-color labels
        TextView copyLabel = view.findViewById(R.id.swap_label);
        if (copyLabel != null) {
            copyLabel.setTextColor(accentColor);
        }
    ...
    /**
     * Renders the detailed Log with custom text into a bitmap
     * @return A bitmap of size 428x142 pixels containing the rendered layout
     */
    public Bitmap renderDetailLogTerminal() {
        // Inflate the layout
        LayoutInflater inflater = LayoutInflater.from(context);
        View view = inflater.inflate(R.layout.detail_log_terminal_layout, null);
    ...
    /**
     * Renders the send NFT layout with custom text into a bitmap
     * Similar to qr_or_send but with "SEND NFT" instead of "SEND TXN"
     * @return A bitmap of size 428x142 pixels containing the rendered layout
     */
    public Bitmap renderSendNft() {
        // Inflate the layout
        LayoutInflater inflater = LayoutInflater.from(context);
        View view = inflater.inflate(R.layout.send_nft_terminal_layout, null);
        ...
        ImageView sendNftIcon = view.findViewById(R.id.send_nft_icon);
        if (sendNftIcon != null) {
            sendNftIcon.setColorFilter(accentColor, PorterDuff.Mode.SRC_IN);
        }
        ...
        TextView sendNftLabel = view.findViewById(R.id.send_nft_label);
        if (sendNftLabel != null) {
            sendNftLabel.setTextColor(accentColor);
        }
